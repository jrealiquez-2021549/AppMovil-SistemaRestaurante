...
export const useReviewStore = create((set, get) => ({
  ...
  fetchReviews: async (restaurantId, page = 1) => {
    try {
      ...
      set({ error: err.response?.data?.message || "Error al cargar reseñas", loading: false });
    }
  },
  ...
  createReview: async (restaurantId, { rating, comment }) => {
    try {
      ...
      set((state) => ({ reviews: [newReview, ...state.reviews], submitting: false }));
      return { success: true };
    } catch (err) {
  ...
  deleteReview: async (reviewId) => {
    try {
      ...
      set((state) => ({ reviews: state.reviews.filter((r) => r._id !== reviewId) }));
      return { success: true };
    } catch (err) {
